...
public class RecipeGenerator extends FabricRecipeProvider {
    ...
    public void generate(Consumer<RecipeJsonProvider> exporter) {
        // Crafting
        // Food & Drinks
        offerShapelessRecipe(exporter, CHICKEN_NUGGETS, Items.CHICKEN, null, 4);
        ...
        offerMultipleShapelessRecipe(exporter, GARLIC_BREAD, 1, FOOD, GROUND_SALT, Items.BREAD, GARLIC);
        ...
        // Smelting
        offerCooking(exporter, BACON, COOKED_BACON);
        offerCooking(exporter, MINCED_BEEF, BEEF_PATTY);
        offerCooking(exporter, CALAMARI, COOKED_CALAMARI);
        offerCooking(exporter, CAT_STEAK, COOKED_CAT_STEAK);
        ...
        offerCooking(exporter, CHICKEN_NUGGETS, COOKED_CHICKEN_NUGGETS);
        offerCooking(exporter, Items.ROTTEN_FLESH, DETOXIFIED_FLESH, false, false, true, false);
        offerCooking(exporter, GARLIC_BREAD, BAKED_GARLIC_BREAD);
        offerCooking(exporter, HORSE_MEAT, COOKED_HORSE_MEAT);
        ...
        offerCooking(exporter, WOLF_MEAT, COOKED_WOLF_MEAT);
    }
    ...
    public static void offerKnifeSlicingRecipe(Consumer<RecipeJsonProvider> exporter, ItemConvertible output, int count, RecipeCategory recipeCategory, ItemConvertible input) {
        ShapelessRecipeJsonBuilder.create(recipeCategory, output, count).input(input).input(DelightfulDishesTags.KNIVES).criterion(hasItem(input), conditionsFromItem(input)).offerTo(exporter);
    }
